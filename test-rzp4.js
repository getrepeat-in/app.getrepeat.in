...
mongoose.connect("mongodb+srv://ajitkushwaha3101:snehavats1404@getrepeat.xjlqc6e.mongodb.net/?appName=getrepeat/development").then(async () => {
  ...
  try {
    ...
    console.log("Refreshed token:", !!res.data.access_token);
    const newToken = res.data.access_token;
    
    // Try to create order
    try {
      const orderRes = await axios.post("https://api.razorpay.com/v1/orders?mode=test", { amount: 50000, currency: "INR" }, {
        headers: { Authorization: `Bearer ${newToken}` }
      });
      console.log("Order success with new token!", orderRes.data);
    } catch (e) {
      console.log("Order fail with new token:", e.response?.data);
    }
