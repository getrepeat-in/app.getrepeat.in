...
mongoose.connect("mongodb+srv://ajitkushwaha3101:snehavats1404@getrepeat.xjlqc6e.mongodb.net/?appName=getrepeat/development").then(async () => {
  ...
  try {
    const res = await axios.post("https://api.razorpay.com/v1/orders", { amount: 50000, currency: "INR", mode: "test" }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    console.log("Body mode=test success:", res.data);
  } catch (e) {
    console.log("Body mode=test fail:", e.response?.data);
  }
